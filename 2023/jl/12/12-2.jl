...
Only difference from part 1 is the reading the file part where we construct the
5x copy, and having to use Memoization.  We need it otherwise it's way too
slow!
"""

@memoize function search_pat(pattern::AbstractString, cons::AbstractArray{Int})
...
function main(filename::AbstractString)
    ...
    for line in eachline(open(filename))
        ...
        pattern = ((pattern * "?") ^ 5)[1:end-1]
        con_s = ((con_s * ",") ^ 5)[1:end-1]
