...
"""
Compute possible neighbours for `current`.  The neighbours must obey the rule
of four to ten.  It must also not return the nwighbour that came before current
in this track.
"""
function neighbours(
    current::GraphNode,
    ...
    for t in ((0, 1), (0, -1), (1, 0), (-1, 0))
        c = CartesianIndex(t)
        #println("c $c current.prev_step $(current.prev_step)")
        # Check if just came from there
        if current.prev_step == CartesianIndex(0, 0) - c
            #println("failed on prev")
            continue
        end
        # Check if not in city
        if any([!(i in a) for (i, a) in zip(Tuple(c + current.loc), axes(city))])
            #println("failed on city")
            continue
        end
        # Check if not acceptable
        if current.num_steps < 4
            if current.prev_step != c && current.prev_step != CartesianIndex(0,0)
                #println("failed on 4")
                continue
            end
        end
        if current.num_steps == 10
            if current.prev_step == c
                #println("failed on 10")
                continue
            end
        end
